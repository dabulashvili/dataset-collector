...
async function download() {
    ...
    const cursor = Record.find({ user: 'tamuna', sentence: { $in: ids } })
        .populate('sentence')
        .cursor()
    let i = 1
    cursor.eachAsync((record) => {
        const id = record._id.toString()
        const wavFile = fs.createWriteStream(
            `/home/davit/data/tts.ge/wavs/${id}.wav`
        )
        https.get(record.url, function (response) {
            response.pipe(wavFile)
        })

        fs.appendFileSync(
            metadata,
            `${id}|${record.sentence.text}|${record.sentence.text}\n`
        )
