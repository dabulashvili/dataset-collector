...
app.get('/list', async (req, res) => {
    ...
    const skips = await Skip.find({
        user: req.user._id,
        sentence: { $in: data.docs.map((s) => s._id) },
    })
...
app.get('/next', async (req, res) => {
    ...
    let nextSentence = await Sentence.findOne({
        _id: { $nin: recordedSentences.concat(skips) },
    }).sort({ order: 1 })
    if (!nextSentence && skips.length) {
        nextSentence = await Sentence.findOne({
            _id: { $nin: recordedSentences },
        }).sort({ order: 1 })
    }
